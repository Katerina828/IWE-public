...
class CWAttack():
    ...
    def CW_loss(self, scores, labels,is_target):
        N, D = scores.shape
        label_mask = F.one_hot(labels, D).type_as(scores)
        correct_logit = torch.sum(label_mask * scores, dim=1)
        #n_label = torch.argmax((1-label_mask)*scores, dim=1)
        #wrong_logit = scores[range(N), n_label]
        wrong_logit = torch.max((1-label_mask)*scores, dim=1)[0]
        
        if is_target==True:
            loss = F.relu(wrong_logit- correct_logit + self.CONFIDENCE)
        else:
        ...
        if is_target==True:
            success_flag = torch.where(wrong_logit<correct_logit, ones, zeros)
        else:
            success_flag = torch.where(wrong_logit>correct_logit, ones, zeros)
        return loss, success_flag
    ...
    def attack_batch(self, net, inputs, labels):
        ...
        v = self.anti_reparameterize(x).clone().detach().to(x.device) #convert x to tanh-space
        ...
        while abs(prev_mean_norm - mean_norm) >= self.TOLERANCE or acc <= 0.99:
            ...
            for t in range(self.T):
                ...
                loss2, success_flag = self.CW_loss(scores, labels,self.is_target)
                #if t%100==0:
                #    print("loss1",torch.mean(loss1).item(),"loss2",torch.mean(loss2).item())
        ...
        return best_adv_x,best_adv_norm, labels
