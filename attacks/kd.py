...
class BaseTrainer(mt.BaseTrainer):
    def __init__(self,teacher_trainer):
        ...
        if self.model_arch =='resnet':
            ...
            self.epochs = 100
        elif self.model_arch =='pre_resnet34' and self.dataset=='caltech101':
            ...
            self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optims['C'], milestones=[50,75], gamma=0.1)
            self.epochs = 100
        elif self.model_arch =='pre_resnet' and self.dataset=='caltech101':
    ...
    def train(self,epoch):
        ...
        for i, (x, y) in enumerate(self.train_loader):
            ...
            if self.loss_type == 'kd':
                ...
                y = F.softmax(teacher_outputs,dim=1)
                loss = torch.mean(torch.sum(-y * F.log_softmax(scores, dim=1), dim=1))
            elif self.loss_type == 'label': 
                y = torch.argmax(teacher_outputs,dim=1)
...
class KDTrainer(BaseTrainer, Confs0):
    def __init__(self,dataset,model_arch,model_type,teacher_trainer,train_loader,loss_type,manual_seed,alpha=0.5,T=1.5):
        ...
        steal_pct = len(train_loader.dataset)/len(teacher_trainer.train_loader.dataset)
        print("KD data pct:", round(steal_pct,2))
        ...
        self.flag = f'clf-{self.dataset}-{model_arch}-{model_type}-stealpct{round(steal_pct,2)}-seed{manual_seed}'
