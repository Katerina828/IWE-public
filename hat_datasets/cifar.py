...
def load_cifar10(
    downsample_pct: float = 0.5, train_pct: float = 0.8, steal_pct:float = 0.2, batch_size: int = 50, img_size: int = 32, label_list: list = None, is_norm=False
    ...
    if is_norm:
        ...
        print("Open RandomCrop and RandomHorizontalFlip")
        train_transform = transforms.Compose(
            [
            transforms.Resize(img_size),
            transforms.RandomCrop(32, padding=4),
    ...
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=2,pin_memory=True, drop_last=True,prefetch_factor=4)
    if train_pct < 1.0:
        valid_loader = DataLoader(valid_set, batch_size=batch_size, shuffle=True, num_workers=2,pin_memory=True)
    else:
        valid_loader = None
        
    subset_index = get_suffle_index(len(train_set))
    downsampled_num_trainpart_examples = int(steal_pct * len(train_set))
    train_set_pct20 = torch.utils.data.Subset(
        train_set, indices=subset_index[0:downsampled_num_trainpart_examples]
    )
    steal_loader = DataLoader(train_set_pct20, batch_size=batch_size, shuffle=True, num_workers=8)
    ...
    return train_loader, valid_loader, test_loader, steal_loader


def load_cifar100(
    downsample_pct: float = 0.5, train_pct: float = 0.8,steal_pct:float = 0.2, batch_size: int = 50, label_list: list = None, is_norm=False
    ...
    cifar100_dataset = torchvision.datasets.CIFAR100(
        root=DATASET.CIFAR100_ROOT, train=True, download=True, transform=train_transform
    )
    if label_list is not None: 
        train_valid_set = torch.utils.data.Subset(cifar100_dataset, [idx for idx, label in enumerate(cifar100_dataset.targets) if label in label_list])
    else:
        train_valid_set = cifar100_dataset
    # Partition into training/validation
    downsampled_num_examples = int(downsample_pct * len(train_valid_set))
    n_train_examples = int(train_pct * downsampled_num_examples)
    n_valid_examples = downsampled_num_examples - n_train_examples
    ...
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=8,pin_memory=True,pin_memory_device='cuda')
    if train_pct < 1.0:
        valid_loader = DataLoader(valid_set, batch_size=batch_size, shuffle=True, num_workers=8,pin_memory=True,pin_memory_device='cuda')
    else:
        valid_loader = None
    ...
    steal_loader= DataLoader(train_set_pct20, batch_size=batch_size, shuffle=True, num_workers=8,pin_memory=True)

    # Load test set
    # pyre-fixme[16]: Module `datasets` has no attribute `MNIST`.
    test_set_all = torchvision.datasets.CIFAR100(
        root=DATASET.CIFAR100_ROOT, train=False, download=True, transform=eval_transform
    )
    downsampled_num_test_examples = int(downsample_pct * len(test_set_all))
